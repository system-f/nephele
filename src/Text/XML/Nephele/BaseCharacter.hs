...
module Text.XML.Nephele.BaseCharacter(
  BaseCharacter
, baseCharacter
, baseCharacters
, baseCharacters1
...
--
-- >>> parse baseCharacter "test" "abc"
-- Right (BaseCharacter 'a')
baseCharacter ::
  CharParsing m =>
  m BaseCharacter
baseCharacter =
...
-- | Parse zero or many baseCharacters.
baseCharacters ::
  CharParsing m =>
  m [BaseCharacter]
baseCharacters =
...
-- | Parse one or many baseCharacters.
baseCharacters1 ::
  CharParsing m =>
  m (NonEmpty BaseCharacter)
baseCharacters1 =
  some1 baseCharacter

-- | BaseCharacter prism from a char.
--
-- >>> 'a' ^? baseCharacter'
-- Just (BaseCharacter 'a')
baseCharacter' ::
  Prism' Char BaseCharacter
baseCharacter' =
  prism'
    (\(BaseCharacter c) -> c)
    ((^? _Right) . parse baseCharacter "baseCharacter'" . singleton)

-- | BaseCharacter prism from text.
--
-- >>> pack "abc" ^? baseCharacters'
-- Just (BaseCharacter 'a')
baseCharacters' ::
  Prism' Text BaseCharacter
baseCharacters' =
  prism'
    (\(BaseCharacter c) -> singleton c)
    ((^? _Right) . parse baseCharacter "baseCharacter'")
