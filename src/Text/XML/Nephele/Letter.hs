...
import Text.XML.Nephele.BaseChar(BaseChar, parseBaseChar)
...
letter ::
  ...
  m Letter
letter =
  (BaseCharacterLetter <$> parseBaseChar) <|> (IdeographicLetter <$> ideographic)
...
letter' ::
  Iso' Letter (Either BaseChar Ideographic)
letter' =
  iso (\l -> case l of
